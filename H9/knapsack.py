...
def knapsack(items: List[Item], max_capacity: int) -> List[Item]:
    ...
    # rows: 1 + #items, cols: 1 + max_capacity (row-0 = 0, col-0 = 0)
    table: List[List[float]] = [
        [0.0 for _ in range(max_capacity + 1)] for _ in range(len(items) + 1)
    ]
    for i, item in enumerate(items, start=1):
        for capacity in range(1, max_capacity + 1):
            previous_items_value: float = table[i-1][capacity]
            value_freeing_weight_for_item: float = table[i-1][capacity - item.weight]     # noqa: E501
            if capacity >= item.weight:  # item fits in knapsack
                ...
                table[i][capacity] = previous_items_value
