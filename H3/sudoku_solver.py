...
def filehelper(filename: str):
    """Read the given input into a list of lists, where each list represents
    the elements in each row.
    Replace None if no value was given (coded as '-' in the input file).
    Sudoku cells must be separated by a single space, every line represents
    a row.
    """
    with open(filename) as sudoku_file:
        all_lines = sudoku_file.readlines()
    return [
        [int(i) if i != "-" else None for i in line.strip().split(" ")]
    ...
    def __init__(self, cells: list[list[int]]) -> None:
        ...
        for i in range(len(cells)):
            assert len(cells) == len(
                cells[i]
            ), f"Row {i} has length {len(cells[i])} - expected {len(cells)}"
        self.sudoku = cells
        self._dim = len(cells)
...
if __name__ == "__main__":
    ...
    lines: list[tuple[int, int]] = filehelper(
        "classic-computer-science-problems-in-python/H3/" "sudokuonline_20230518.txt"
    )
